...
def test_options_call(base64_client, base64_header, mock_epdb_serve):
    ...
    result = base64_client.simulate_options(headers={
        'X-EPDB': 'Base64 {}'.format(base64_header)
    })
...
def test_header_exception(base64_client, base64_header, mock_epdb_serve):
    ...
    assert mock_epdb_serve.called
    logs.check_present(
        ('falcon_epdb', 'ERROR', 'Attempted, but failed, to serve epdb: Oops',))
...
def test_invalid_header_data_not_a_dict(base64_client, mock_epdb_serve):
    ...
    logs.check_present((
        ...
        ' Invalid X-EPDB content; must be a dictionary',))
...
def test_base64_invalid_header_needs_two_tokens(
        base64_client, base64_header, mock_epdb_serve):
    """Test that we expect the header to contain a scheme and payload."""

    with testfixtures.LogCapture() as logs:
        result = base64_client.simulate_get(headers={
            'X-EPDB': '{}'.format(base64_header)
        })
...
def test_base64_invalid_header_needs_correct_scheme(
        base64_client, base64_header, mock_epdb_serve):
    """Test that we expect the header to contain the Base64 scheme."""

    with testfixtures.LogCapture() as logs:
        result = base64_client.simulate_get(headers={
            'X-EPDB': 'Base32 {}'.format(base64_header)
        })
    ...
    logs.check_present((
        ...
        'ERROR',
        'Attempted, but failed, to serve epdb:'
        ' Invalid X-EPDB value; scheme must be Base64',))
